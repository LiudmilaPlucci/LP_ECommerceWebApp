            ...
            for (const [orderId, orderData] of orderSnapshots) {
                ...
                if (productSnapshot.exists()) {
                    ...
                    for (const product of products) {
                        ...
                        const productDetailsSnapshot = await productsRef.child(product.name + "-" + productId).once('value');
                        ...
                        if (productDetails) {
                            ...
                            console.warn(`Product details not found for: ${product.name}-${productId}`);
                        }
                    }
...
function updateCartCount() {
    ...
    // Fetch active order
    db.ref('orders').orderByChild('active').equalTo('true').once('value', snapshot => {
        console.log(snapshot.val());
        if (snapshot.exists()) {
            snapshot.forEach(orderSnapshot => {
                let orderId = orderSnapshot.key;
                /*     db.ref('orders/' + orderId).update({ active: false }); */

                // Fetch the current cart count from the active order
                db.ref('orders/' + orderId + '/products').once('value', productSnapshot => {
                    if (productSnapshot.exists()) {
                        productSnapshot.forEach(childSnapshot => {
                            cartProductTotalCount += childSnapshot.val().amount;
                        });
                    }

                    // Update the cart count display
                    if (cartProductTotalCount > 0) {
                        cartCountElement.textContent = cartProductTotalCount;
